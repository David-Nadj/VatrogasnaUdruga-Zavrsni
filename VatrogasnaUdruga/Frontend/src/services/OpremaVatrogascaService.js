...
async function urediVezu(veza){
    return await HttpService.put('/OpremaVatrogasca/urediVezu',veza)
    .then((odgovor)=>{
        return {greska: false, poruka: odgovor.data}
    })
    .catch((e)=>{
        switch (e.status) {
            case 400:
                ...
                return {greska: true, poruka: poruke}
            default:
                return {greska: true, poruka: 'Veza oprema vatrogasca se ne može promjeniti!'}
        }
    })
}

async function pronadiIUkloni(oprema) {
    return await HttpService.delete('/OpremaVatrogasca/ukloni', { data: oprema })
        .then(() => {
            return { greska: false, poruka: 'Obrisano' };
        })
        .catch(() => {
